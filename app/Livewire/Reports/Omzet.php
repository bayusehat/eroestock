...
class Omzet extends Component
{
    ...
    {
        $omzet = DB::select("select order_sn, buyer_username, create_time, escrow_amount, gross_profit
            from(
            select *, case when escrow_amount <> 0 then escrow_amount - buy_price else 0 end as gross_profit from (
            select order_sn, buyer_username, create_time, escrow_amount, b.model_discounted_price, b.model_sku from shopee_orders a
